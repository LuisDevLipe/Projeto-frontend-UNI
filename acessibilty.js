...
const add_rm_animation = async () => {
	await sleep(500)
	acessibilidadeElem.classList.add("animar-area-acessibilidade")
	await sleep(4000)
	acessibilidadeElem.classList.remove("animar-area-acessibilidade")
}
...
document.addEventListener("DOMContentLoaded", function () {
	if (getCookieValue() || getCookieValue() == null) {
		...
		acessibilidadeElem.focus()
		setCookie("modalAcessibilidadeShow", 0) // ao carregar se o cookie existir reseta o cookie para nao aparecer por mais 1 dia
		add_rm_animation()
		return
	}
})
...
function setCookie(cookie = "modalAcessibilidadeShow", value = 1, expires = 86400000) {
	//expires default is 1 day
